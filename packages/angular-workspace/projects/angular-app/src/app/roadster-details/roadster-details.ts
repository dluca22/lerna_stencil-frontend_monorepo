...
@Component({
  selector: 'app-roadster-details',
  templateUrl: './roadster-details.html',
  styleUrls: ['./roadster-details.css']
})
export class RoadsterDetailsComponent {
  @Input() roadster: Roadster = {
  ...
  fallback(value: any, defaultVal: string = '--'): string {
    return value !== undefined && value !== null ? value.toString() : defaultVal;
  }
  ...
  get thumbnail(): string {
    return this.roadster?.flickr_images?.[0] || '';
  }
}